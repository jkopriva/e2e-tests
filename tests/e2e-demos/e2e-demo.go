			...
			for _, componentTest := range appTest.Components {
				...
				if componentTest.ContainerSource != "" {
					...
					It(fmt.Sprintf("creates component %s from %s git source %s git revision %s", componentTest.Name, componentTest.Type, componentTest.GitSourceUrl, componentTest.GitSourceRevision), func() {
						component, err = fw.HasController.CreateComponentFromDockerfile(application.Name, componentTest.Name, namespace,
							componentTest.GitSourceUrl, componentTest.GitSourceRevision, componentTest.GitDockerfileUrl, "", containerIMG, oauthSecretName, false)
						Expect(err).NotTo(HaveOccurred())
					})
