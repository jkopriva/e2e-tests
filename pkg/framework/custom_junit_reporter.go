...
func GenerateRPPreprocReport(report types.Report) {
	for i := range report.SpecReports {
		...
		if reportSpec.LeafNodeType == types.NodeTypeIt {
			...
			labels := reportSpec.Labels()
			if len(labels) > 0 {
				//name = name + " [" + strings.Join(labels, ", ") + "]"
			}
			filePath := "rp_preproc/attachements/" + name
			if err := os.MkdirAll(filePath, os.ModePerm); err != nil {
				klog.Fatal(err)
			} else {
				writeLogInFile(filePath+"/ginkgoWriterLog.txt", reportSpec.CapturedGinkgoWriterOutput)
				writeLogInFile(filePath+"/stdOutErrLog.txt", reportSpec.CapturedStdOutErr)
				writeLogInFile(filePath+"/failureMessageLog.txt", reportSpec.FailureMessage())
				writeLogInFile(filePath+"/failureLocationLog.txt", reportSpec.FailureLocation().FullStackTrace)
			}
			//}
		}
	}
